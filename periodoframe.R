...
MP <- function(t, y, dy,Dt,nbin,fmax=1,ofac=1,fmin=1/1000,tspan=NULL,per.type='bgls',...){
    ...
    rel.powers <- powers <- array(data=NA,dim=c((fmax-fmin)/df+dNf,nbin))
    ndata <- rep(NA,nbin)
    for(j in 0:n){
        cat(paste0(Dt,'d window'),j+1,'/',nbin,'\n')
        inds <- which(t>=min(t)+j*dt & t<min(t)+j*dt+Dt)
        if(per.type=='bgls'){
            tmp <- bgls(t=t[inds],y=y[inds],err=dy[inds],fmax=fmax,ofac=ofac,fmin=1/Dt,tspan=Dt)
        }else if(per.type=='gls'){
            tmp <- gls(t=t[inds],y=y[inds],err=dy[inds],fmax=fmax,ofac=ofac,fmin=1/Dt,tspan=Dt)
        }else if(per.type=='glst'){
            tmp <- glst(t=t[inds],y=y[inds],err=dy[inds],fmax=fmax,ofac=ofac,fmin=1/Dt,tspan=Dt)
